...
- 係數 1.2（斜率）是透過「讓預測誤差平方和最小」算出來的——這需要**微分** [@armitage2001statistical]
- 標準誤的計算涉及**積分**
- p 值是 t 分布曲線下的面積——也是**積分** [@casella2002statistical]
...
- **Hazard ratio** 和 **hazard function** [@collett2015modelling]
- **Odds ratio** 的 confidence interval（為什麼要取 log？）[@agresti2018introduction]
- **Maximum likelihood estimates** 的 standard error [@pawitan2013all]
...
p3 <- ggplot(data.frame(time, surv, hazard)) +
  ...
  annotate("text", x = 5, y = 0.75, label = "S(t) 存活函數", color = "#2E86AB", hjust = 0) +
  annotate("text", x = 5, y = 0.45, label = "h(t) = -S'(t)/S(t) 危險函數", color = "#E94F37", hjust = 0) +
  labs(title = "存活分析中的導數",
       subtitle = "Hazard function 與 S(t) 的導數相關",
       x = "時間", y = "") +
  theme_minimal(base_size = 12) +
...
微積分不是要讓你變成數學家 [@grinstead2012introduction]。我們的目標是：
